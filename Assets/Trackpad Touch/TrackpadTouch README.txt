...
would use Input.touches:
  ...
  public class TrackpadTest : MonoBehaviour {
      public void Update() {
          foreach (var i in TrackpadInput.touches.Count) {
              var touch = TrackpadInput.touches[i];
              var deviceSize = TrackpadInput.deviceSizes[i]; // Vector2 of device size in points (72ppi)
              switch (touch.phase) {
